...
export class OrderController {
    ...
    @Get('/')
    @UseGuards(JwtAuthGuard )
    getOrdersByManager(@Req() req ,@Query('page') page:number, @Query('managerId') id:number  ,@Query('waiterId') waiterId:number  ) : Promise<Paginated<Order>> {
        const user = req.user;
        if( (id && waiterId ) || !id && !waiterId)
            throw new BadRequestException("Must include only one of [managerId , waiterId]");

...
export class OrderService {
  ...
  async createOrder(dto: OrderDto, waiterId: number): Promise<Order> {
    ...
    let promises = orderItems
      .map((x) => x.menuItem.id)
      .map((x) => this.menuService.isMenuItem(managerId, x));
    let areValidItems: boolean[] = await Promise.all(promises);
    const allTrue = areValidItems.every((value) => value === true);

    if (!allTrue) throw new BadRequestException('Invalid items found');
  ...
  async getOrder(orderId: number): Promise<Order> {
    ...
    return {
      ...order,
      waiter: { id: order.waiter.id, name: order.waiter.name } as Waiter,
    };
  }
  ...
  async getOrders(
    ...
    if (waiterId) return this.getOrdersWaiter(waiterId, pageNumber);

    const take = constants.PAGE_DEFAULT_SIZE;
    const skip = pageNumber * take;
    console.log(pageNumber, skip, take);

    const [result, count] = await this.orderRepository
      .createQueryBuilder('order')
      .select('order')
      .leftJoinAndSelect('order.waiter', 'linkedWaiter')
      .leftJoinAndSelect('linkedWaiter.manager', 'linkedManager')
      .where('linkedWaiter.manager = :managerId', { managerId })
